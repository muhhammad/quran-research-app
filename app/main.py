...
from app.analysis import verse_analysis, letter_analysis, word_search, muqattaat_analysis
...
def verse_analyze(request: Request, surah: int = Form(...), ayah: int = Form(...)):
    ...
    # ✅ Dynamic ayah validation (THIS is important)
    max_ayah = AYAH_COUNTS.get(surah, 0)
    ...
    if ayah < 1 or ayah > max_ayah:
        return templates.TemplateResponse(
            ...
            {
                "request": request,
                "error": f"Invalid Ayah for Surah {surah} (max {max_ayah})",
                "surahs": SURAH_NAMES,
                "ayah_counts": AYAH_COUNTS
            }
        )
    ...
    return templates.TemplateResponse(
        "verse.html",
        {
            ...
            "result": result,
            "surahs": SURAH_NAMES,
            "ayah_counts": AYAH_COUNTS   # ✅ CRITICAL FIX
        }
    )
